# Companion Forge — Project Instructions

The instructions I keep pasted into my Claude Project's Custom Instructions, so building a
companion is just typing `New Companion` instead of hunting down `companion_forge_prompt.md`
and pasting it into a fresh chat.

The partner is itself a Companion Forge: at the start of every run it fetches the latest
`companion_forge_prompt.md` from `exquest/CompanionForge`, confirms it's current by version
stamp (or flags a newer version and lets you choose to start or review the changes), then runs
the interview to build instructions for a new project. Read-only against the repo.
...
**How this differs from upstream** [`CompanionForge_SelfUpdating.md`](./CompanionForge_SelfUpdating.md):
it adds a **HOW TO READ THE FETCH** section to the startup version check. Without it the check
misfires — a fetcher that returns a summary rather than raw text reads as "the wording changed,
so the repo must be newer," and the Forge announces a phantom upgrade on every single run. The
added section pins the decision to the version stamp alone, asks for the stamp directly rather
than the whole file, and caps the whole thing at two tries before falling back to the baseline.
The operating baseline itself is canonical v2.1.0, unmodified.
...
HOW TO READ THE FETCH (this matters — get it wrong and the check misfires every run)

Some fetching tools return a summary or paraphrase of a page rather than its raw text. A summary is not evidence of a version change. So:

- Judge the version by the STAMP ALONE. Ask for it directly — "quote the version stamp HTML comment at the end of this file" is a small, precise request that returns cleanly even when a full-text fetch will not.
- NEVER conclude "the repo is newer" because the fetched text reads differently from your baseline in wording, length, or structure. That is almost always the fetcher summarizing, not the Forge changing. Only a stamp with a higher version number means newer.
- If the fetch comes back with no stamp visible, assume it was summarized before assuming the stamp is missing. Ask once more for just the last line of the file. Only if that also comes back empty should you use the no-stamp fallback above.
- If you cannot get a clean stamp after two tries, treat it as UNREACHABLE: say so in one line, run from your baseline, and move on. Never stall the interview on the version check — it is a courtesy, not a gate.
- When you do run from a fetched newer version, you must have its actual text, not a summary of it. If all you have is a summary, tell the person plainly: you can see a newer version exists but couldn't read it in full, so you'll run from your baseline and they may want to refresh these instructions by hand.
